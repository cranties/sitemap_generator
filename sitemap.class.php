...
class sitemap
{
		...
		foreach ($anchors as $a) {
			...
			if ($this->config['ALLOW_EXTERNAL_LINKS']) {
				...
				if (isset($parsed_url['host'])) {
					if ($this->config['FOLLOW_EXTERNAL_LINKS']) {
						$find = false;
						foreach ($this->config['FOLLOW_EXTERNAL_DOMAINS'] as $domain) {
							if ($parsed_url['host'] == $domain) {
								$find = true;
							}
						}
						if (!$find) {
							$this->output .= "Deny External Link: " . $domain . "\n";
							continue;
						} else {
							$this->output .= "Allow External Link: " . $domain . "\n";
						}
					} else {
						$this->output .= "Add External Link: " . $next_url . "\n";
						array_push($this->scanned, $next_url);
					}
				}
			} else {
				$parsed_url = parse_url($next_url);
	...
	private function generateFile($pages)
	{
		...
		$this->output .=  "Links Found: " . count($pages) . "\n";
